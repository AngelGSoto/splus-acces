...
def main():
    ...
    try:
        ...
        # Convert the first 100 rows to pandas DataFrame
        dual_sqg_head = dual_sqg.head(100)
        dual_sqg_df = pd.DataFrame(dual_sqg_head)

        # Save the first 100 rows
        dual_sqg_df.to_csv('dual_sqg_first_100.csv', index=False)
        print("Las primeras 100 filas del resultado del crossmatch se han guardado en 'dual_sqg_first_100.csv'")
    except Exception as e:
        print("Error durante el guardado de las primeras 100 filas del crossmatch:", e)
